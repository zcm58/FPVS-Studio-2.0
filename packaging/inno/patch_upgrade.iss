...
begin
  ...
  if not Result then
    MsgBox('This patch requires an existing FPVS Studio installation. Use the full installer.',
      mbError, MB_OK);
end;
...
var
  ...
  Path: String;
begin
  Result := 'This installation does not match the patch source. Use the full FPVS Studio installer to repair or update it.';
...
begin
  ...
  if not Result then begin
    ...
    SuppressibleMsgBox('The patch needs repair. Run this patch again or use the full FPVS Studio installer. Windows may already show the new version, but the application files could not be verified.',
      mbError, MB_OK, IDOK);
  end;
end;
...
begin
  if (CurPageID = wpFinished) and PatchVerificationFailed then begin
    WizardForm.FinishedHeadingLabel.Caption := 'FPVS Studio update needs repair';
    WizardForm.FinishedLabel.Caption := 'The application files could not be verified. Run this patch again or use the full installer before opening FPVS Studio.';
