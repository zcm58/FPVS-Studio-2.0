...
| 3 | GUI workflow docs | Docs/GUI | Keep Welcome/Home, raw import, normalization, and runtime launch docs synchronized with the implemented GUI. | `.\.venv3.10\Scripts\python -m pytest -q tests\unit\test_harness_docs.py` | Passing |
...
- Lint status: `.\.venv3.10\Scripts\python -m ruff check src tests` passes.
- Type status: `.\.venv3.10\Scripts\python -m mypy src` currently needs the Windows/PsychoPy verification
