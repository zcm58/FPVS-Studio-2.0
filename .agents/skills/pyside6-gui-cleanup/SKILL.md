...
13. Add or update focused pytest-qt coverage for geometry and text clipping at the
    documented minimum/default size and register its module in
    `tests/qt_test_files.txt`; do this during the initial implementation, not as a later
    visual-polish pass.
...
- Include the visible/manual smoke path and state whether registered pytest-qt coverage
  was not run or ran in an explicitly approved visible environment.
