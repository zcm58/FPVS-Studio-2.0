...
def _check_metadata(bundle: Path, version: str) -> None:
    metadata = list((bundle / "_internal").glob("fpvs_studio-*.dist-info/METADATA"))
    if len(metadata) != 1:
        raise InventoryError("Patch target requires one complete bundled app metadata file.")
    lines = metadata[0].read_text(encoding="utf-8").splitlines()
    if [line for line in lines if line.startswith("Version: ")] != [f"Version: {version}"]:
...
def prepare_patch(
    *,
    ...
    result: dict[str, object] = {
        ...
        "asset_name": f"FPVS-Studio-Patch-{source_version}-to-{target_version}.exe",
...
def build_release_manifest(
    *, target_version: str, installer_dir: Path, patch_builds: list[Path]
    ...
    full = installer_dir / f"FPVS-Studio-Setup-{target_version}.exe"
    ...
    for build_path in patch_builds:
        ...
        versions.add(source)
        name = f"FPVS-Studio-Patch-{source}-to-{target_version}.exe"
    ...
    output = installer_dir / f"FPVS-Studio-Update-{target_version}.json"
    output.write_text(
        json.dumps(
            {
                "schema_version": 1,
