...
No FPVS Studio executable, Qt application, or production installer is launched.
"""
...
PRODUCTION_APP_ID = "C0EAFB18-1DC5-4C77-8FDB-F6C1E7874694"
...
def make_bundle(root: Path, version: str, *, removed: str, added: str) -> Path:
    ...
    payloads = {
        "FPVS Studio.exe": f"INERT NON-EXECUTABLE FIXTURE {version}\n",
        "_internal/unchanged.txt": "unchanged shared dependency\n",
        f"_internal/{added}": f"added in {version}\n",
        f"_internal/fpvs_studio-{version}.dist-info/METADATA": (
...
def main() -> int:
    ...
    report: dict[str, object] = {
        ...
        "source_sha256": {
            ...
            for name in (
                ...
                "packaging/inno/patch_upgrade.iss",
                "packaging/inno/owned_files.iss",
                "packaging/inno/updater_cache.iss",
                "scripts/build_patch.py",
                "scripts/check_patch_installer_lifecycle.py",
            )
        },
        "steps": steps,
        "ok": False,
    ...
    def compile_setup(
        ...
        *,
        ...
        command.append(str(repo / "packaging/inno/fpvs_studio.iss"))
    ...
    try:
        ...
        base_exe = bundles["0.0.1"] / "FPVS Studio.exe"
            ...
            json.dumps(
                {
                    ...
                    "files": [{"path": "FPVS Studio.exe", "sha256": [sha256_file(base_exe)]}],
                }
            ),
            encoding="utf-8",
        )
        ...
        steps.append("fresh base install")
        sentinels = [
            ...
            fixture / "user-data/.fpvs-studio/templates/keep.txt",
        ]
        ...
        replaced = installed / "FPVS Studio.exe"
        ...
        unrelated_markers = {
            ...
            "unrelated-fingerprints": (
                "FPVS-STUDIO-PATCH-TRANSACTION-1\r\nfrom=0.0.1\r\nto=0.0.2\r\n"
                f"source={'a' * 64}\r\ntarget={'b' * 64}\r\n"
            ).encode("utf-8-sig"),
        }
        ...
        steps.append("uninstall removed owned files and retained all six user-data sentinels")
        report["ok"] = True
