"""Installer download helpers for the FPVS Studio updater."""
...
from fpvs_studio.updates.models import DownloadedInstaller, InstallerAsset, UpdateError
...
def default_update_cache_dir() -> Path:
    ...
    if local_app_data:
        return Path(local_app_data) / "FPVS Studio" / "updates"
    return Path(tempfile.gettempdir()) / "FPVS Studio" / "updates"
...
def download_installer(
    ...
    *,
    ...
    request = Request(
        asset.download_url,
        headers={"User-Agent": f"FPVS-Studio/{__version__}"},
    )
    try:
