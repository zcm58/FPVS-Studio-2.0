...
replace the bundled helper together with Studio. No project directory is involved.
"""
...
from fpvs_studio.updates.cache import default_update_cache_dir, discard_file, verified_installer
...
from fpvs_studio.updates.github_releases import DEFAULT_RELEASES_API_URL, fetch_release_metadata
...
from fpvs_studio.updates.validation import parse_release_version, validate_asset_identity

UPDATER_FILENAME = "FPVS Studio Updater.exe"
STUDIO_FILENAME = "FPVS Studio.exe"
_UNINSTALL_KEY = (
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall"
    r"\{C0EAFB18-1DC5-4C77-8FDB-F6C1E7874694}_is1"
)
...
def registered_installation() -> InstalledApplication | None:
    """Read Studio's registration, independently of the helper's own version."""
    ...
    if (
        ...
        raise UpdateError("Studio's installation registration is invalid.")
    parse_release_version(version)
    root = validate_cache_path(Path(location))
    with guarded_read_directory(root):
        pass
    return InstalledApplication(root, version)
...
def helper_command(*arguments: str) -> list[str]:
    ...
    if not getattr(sys, "frozen", False):
        return [sys.executable, "-m", "fpvs_studio.updater_main", *arguments]
    executable = Path(sys.executable)
    if executable.name == STUDIO_FILENAME:
        ...
        raise UpdateError("This executable is not a supported Studio updater.")
...
def spawn_helper_command(command: list[str], *, stdio: bool = False) -> subprocess.Popen[bytes]:
    ...
    try:
        ...
        raise UpdateError(f"Could not start FPVS Studio Updater: {error}") from error
...
def _process_image(kernel: Any, handle: Any) -> Path:
    ...
    if not kernel.QueryFullProcessImageNameW(handle, 0, buffer, ctypes.byref(length)):
        raise UpdateError("Could not identify the Studio process before updating.")
    return Path(buffer.value)


class ParentProcess:
    """Hold a process handle, never a reusable PID, throughout Studio shutdown."""
    ...
    def __init__(self, pid: int, expected_executable: Path) -> None:
        if sys.platform != "win32" or type(pid) is not int or pid <= 0 or pid == os.getpid():
            raise UpdateError("The updater received an invalid Studio process identity.")
        self._kernel = _windows_api()
        self._handle = self._kernel.OpenProcess(0x100000 | 0x1000, False, pid)
        if not self._handle:
            raise UpdateError("Studio closed before the updater could accept the handoff. Retry.")
        try:
            if _process_image(self._kernel, self._handle) != expected_executable:
                raise UpdateError("The updater handoff does not identify the installed Studio.")
    ...
    def wait(self, cancel_event: Event | None = None) -> None:
        ...
        while True:
            ...
            if state == 0:
                return
            if state != 258:
                raise UpdateError("The updater could not wait for Studio to close.")
            if time.monotonic() - started > PARENT_EXIT_TIMEOUT_SECONDS:
                raise UpdateError("Studio is still open. Close it and retry the update.")
...
def studio_is_running(executable: Path) -> bool:
    """Check other exact-path Studio processes without closing any application."""
    if sys.platform != "win32":
        raise UpdateError("Installing Studio updates requires Windows.")
    import ctypes
    from ctypes import wintypes
    ...
    if not psapi.EnumProcesses(ids, ctypes.sizeof(ids), ctypes.byref(count)):
        raise UpdateError("Could not check whether Studio is still open.")
...
class PreparedInstall:
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            # installer is still modifying Studio or arranging its restart.
            with locked_cache(
    ...
    def _run_locked(
        ...
        *,
        ...
        try:
            if self.parent is not None:
                _phase(phase_callback, "Waiting for FPVS Studio to close...")
                self.parent.wait(cancel_event)
            check_cancel(cancel_event)
            current = registered_installation()
            if current != self.installation:
                raise UpdateError("Studio's installation changed. Check for updates again.")
            executable = self.installation.root / STUDIO_FILENAME
            if studio_is_running(executable):
                raise UpdateError("Another Studio window is open. Close it and retry the update.")
            ...
            try:
                _phase(
                    phase_callback,
                    "Installing FPVS Studio. Please keep this window open...",
            ...
            if code != 0:
                suffix = (
                    " Use Download Full Installer to repair Studio."
                    if self.downloaded.asset.kind == "patch"
                    else " Retry the full installer to repair Studio."
                )
            ...
            if (
                ...
                raise UpdateError(
                    "Setup finished, but the expected Studio version was not registered."
                )
            _phase(phase_callback, "Update installed. Restarting FPVS Studio...")
            with guarded_read_path(executable):
                try:
                    launch_independent_process(
                        [str(executable)], close_fds=True, cwd=str(executable.parent)
                    )
                except OSError as error:
                    raise UpdateError(
                        "Studio was updated successfully but could not restart. Open it from Start."
                    ) from error
        finally:
            self.close()
...
def prepare_install(
    ...
    *,
    ...
    """Authenticate a handoff and pin Studio's process before acknowledging ready."""
    check_cancel(cancel_event)
    validate_asset_identity(downloaded.asset)
    installation = registered_installation()
    if installation is None:
        raise UpdateError("Install Studio using its full installer before using Update & Repair.")
    target = downloaded.asset.version
    if target is None or parse_release_version(target) < parse_release_version(
        installation.version
    ):
        raise UpdateError("The updater will not install an older Studio version.")
    if downloaded.asset.kind == "patch" and downloaded.asset.from_version != installation.version:
        raise UpdateError(
            "This patch is for a different installed version. Check for updates again."
        )
    if (
        ...
        raise UpdateError("The installer handoff does not match Studio's verified update cache.")
    _authenticate_release_asset(downloaded, cancel_event)
    parent = (
        ParentProcess(parent_pid, installation.root / STUDIO_FILENAME)
...
def _authenticate_release_asset(
    downloaded: DownloadedInstaller, cancel_event: Event | None
) -> None:
    ...
    for release in fetch_release_metadata(DEFAULT_RELEASES_API_URL, cancel_event=cancel_event):
        ...
        if not isinstance(raw_assets, list):
            continue
        matches.extend(
            item for item in raw_assets if isinstance(item, dict) and item.get("name") == asset.name
        )
    if len(matches) != 1:
