...
setup and for a restarted Studio with a different bundle.
"""
...
from fpvs_studio.updates.models import UpdateError
    ...
    with _DLL_SEARCH_LOCK:
        ...
        try:
            ...
            if not kernel.SetDllDirectoryW(previous):
                _LOG.error(
                    "updater_dll_search_restore_failed", extra={"error": ctypes.get_last_error()}
                )


def launch_independent_process(command: list[str], **options: Any) -> subprocess.Popen[bytes]:
    """Spawn one independent helper, native installer, or restarted Studio."""
    if "env" in options:
