...
from fpvs_studio.updates.github_releases import check_update_candidate
from fpvs_studio.updates.helper_protocol import (
    asset_from_dict,
    download_from_dict,
    download_to_dict,
    read_message,
...
from fpvs_studio.updates.helper_runtime import prepare_install, registered_installation
...
def check_update(
    ...
    *,
    ...
    root = (
        installation.root if installation is not None and installation.version == current else None
    )
...
def binary_stdio() -> tuple[BinaryIO, BinaryIO]:
    ...
    for stream, standard, mode in ((sys.stdin, -10, "rb"), (sys.stdout, -11, "wb")):
        ...
        if not original or not kernel.DuplicateHandle(
            process, original, process, ctypes.byref(duplicate), 0, False, 2
        ):
...
def _validate_request(request: dict[str, Any]) -> str:
    ...
    if command == "check":
        ...
        if (
            (
                request["current_version"] is not None
                and not isinstance(request["current_version"], str)
            )
...
def run_apply(
    ...
    *,
    ...
    Studio remains open until acceptance. An early EOF, cancel, failed preparation,
    ...
    try:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise UpdateError("Studio did not accept the update handoff. Retry the update.")
            try:
        ...
        if (
            ...
            raise UpdateError("Studio did not accept the prepared update handoff.")
        check_cancel(cancel)
        watchdog.disarm()
        accepted = True
