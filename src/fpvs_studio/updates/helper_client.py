"""Synchronous subprocess client, intended for Studio's existing Qt workers."""
...
from fpvs_studio.updates.helper_runtime import helper_command, spawn_helper_command
...
class HelperClient:
    """Private child process per operation; Studio owns no network/hash work."""
        ...
        try:
            ...
            while True:
                ...
                if isinstance(message, BaseException):
                    check_cancel(cancel_event)
                    raise UpdateError(
                        "The updater closed before completing the operation."
                    ) from message
                kind = message["kind"]
                if kind == "phase":
    ...
    def launch_install(
        ...
        *,
        ...
        """Return after the staged helper accepts responsibility, before Studio exits."""
        if not relaunch_after_install:
            raise UpdateError("The independent updater completes updates by restarting Studio.")
