"""Bounded JSON-lines messages between Studio and its independent updater.
...
def asset_from_dict(value: Any) -> InstallerAsset:
    ...
    if (
        ...
        or any(
            ...
            for field in ("size_bytes", "asset_id")
        )
    ):
...
def phase_from_message(message: dict[str, Any]) -> UpdatePhase:
    if (
        ...
        or type(message.get("install_committed", False)) is not bool
    ):
