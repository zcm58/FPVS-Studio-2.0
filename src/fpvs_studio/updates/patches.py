    ...
    *,
    ...
    if root is None:
        return replace(
            result,
            selection_reason=(
                "Full installer for this unregistered or unsupported application build."
            ),
        )
    ...
    for asset in sorted(compatible, key=lambda candidate: candidate.size_bytes or 0):
        ...
        try:
            ...
            return replace(
                result,
                selection_reason=(
                    "The installed files do not match this patch. A full update is required."
                ),
            )
        return replace(
            result,
...
def require_running_patch_baseline(
    asset: InstallerAsset, *, cancel_event: Event | None = None
) -> Path:
...
def _fetch_manifest(metadata: dict[str, object], target: str, cancel_event: Event | None) -> object:
    ...
    request = Request(
        url, headers={"Accept-Encoding": "identity", "User-Agent": "FPVS-Studio-Updater"}
    )
    try:
...
def _parse_manifest(
    document: object, assets: Sequence[dict[str, object]], target: str
) -> list[InstallerAsset]:
    if (
    ...
    for item in document["patches"]:
        if (
            ...
            or set(item)
            != {"from_version", "source_inventory_sha256", "asset_name", "size_bytes", "sha256"}
        ...
        if (
            ...
            or item["asset_name"] != f"FPVS-Studio-Patch-{source}-to-{target}.exe"
            or source in seen
        ):
            raise UpdateIntegrityError(
                "The patch source, target, or filename is invalid or duplicated."
            )
        seen.add(source)
        ...
        if (
            ...
            raise UpdateIntegrityError(
                "The patch size, SHA-256, or identity does not match GitHub."
            )
...
def verify_patch_baseline(
    ...
    if hashlib.sha256(raw).hexdigest() != asset.source_inventory_sha256:
        raise UpdateIntegrityError(
            "the installed file inventory does not match the patch baseline."
        )
    try:
    ...
    if (
        lines[:3] != ["FPVS-STUDIO-OWNED-FILES-1", "kind=current", f"version={current_version}"]
...
def _validate_relative_path(value: str) -> None:
    ...
    if (
        ...
        or any(
            part in {"", ".", ".."}
            or part.endswith((".", " "))
            or _WINDOWS_DEVICE.fullmatch(part.split(".", 1)[0])
            for part in parts
        )
