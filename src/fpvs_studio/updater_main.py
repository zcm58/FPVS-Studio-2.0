...
from fpvs_studio.updates.helper_protocol import PROTOCOL_VERSION


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="FPVS Studio Update & Repair")
            ...
            json.dumps(
                {
                    ...
                    "gui_loaded": any(
                        name.startswith("PySide6") or name.startswith("fpvs_studio.gui.")
    ...
    from fpvs_studio.updates.helper_service import serve_stdio

    if args.stdio:
        return serve_stdio()

    from fpvs_studio.updates.helper_runtime import (
        helper_command,
        is_staged_helper,
    ...
    from fpvs_studio.gui.updater_window import run_updater_gui

    return run_updater_gui(
        apply_mode=args.apply, smoke_report=args.gui_smoke, startup_error=startup_error
    )
