"""Read-only presentation for pooled fixation-cross response data."""
...
class FixationCrossDataDialog(QDialog):
    """Load and display the active project's fixation history without modifying it."""

    def __init__(self, *, project_root: Path, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setObjectName("fixation_cross_data_dialog")
        self.setWindowTitle("Fixation Cross Data — FPVS Studio")
    ...
    def _build_results_page(self) -> QWidget:
        ...
        self.conditions_table.setMinimumHeight(140)
    ...
    def _show_loading_state(self) -> None:
        ...
        self.state_title_label.setText("Loading fixation data...")
    ...
    def _show_error_state(self, error: object) -> None:
        ...
        if isinstance(error, FixationDataError):
            LOGGER.warning("Could not load fixation-cross data: %s", error)
        else:
            LOGGER.warning(
                "Unexpected fixation-cross data error (%s): %s",
                type(error).__name__,
                error,
            )
