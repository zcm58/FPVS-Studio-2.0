"""Small standalone updater windows, independent of Studio's main GUI and models."""
...
from fpvs_studio.updates.helper_service import (
    binary_stdio,
    check_update,
    download_update,
    run_apply,
...
class UpdaterWindow(UpdateDialog):
    """Update or explicitly repair the registered installation without opening Studio."""
    ...
    def __init__(
        ...
        self.setWindowTitle("FPVS Studio Update & Repair")
        self.title_label.setText("FPVS Studio Update & Repair")
        self.current_version_label.setText("Current version: checking installed application...")
        self.setMinimumHeight(660)
        self.resize(760, 680)
        self.repair_button = QPushButton("Repair / Reinstall", self.button_box)
        ...
        if self._repair and isinstance(result, UpdateCheckResult) and result.update_available:
            ...
            if asset is not None and asset.sha256 is not None:
                ...
                self.status_label.setText(
                    f"Full installer{size} available to update or repair FPVS Studio.\n\n"
                    "Close all Studio windows before installing. Projects, templates, settings, "
                    "run history, and logs remain outside the install folder."
                )
...
def run_updater_gui(
    *,
    ...
    app.setApplicationName("FPVS Studio Updater")
    ...
    if startup_error is not None:
        QMessageBox.critical(
            ...
            f"{startup_error}\n\n"
            "You can also download the full installer from FPVS Studio's GitHub Releases page.",
        )
        return 1
    window: QDialog
    if apply_mode:
        try:
            ...
            QMessageBox.critical(None, "FPVS Studio Updater", str(error))
            return 1
...
def run_visible_smoke(app: QApplication, report: Path) -> int:
    ...
    window.setWindowTitle("FPVS Studio Updater — visible smoke check")
    window.title_label.setText("Updater smoke check (synthetic data)")
    asset = InstallerAsset(
        name="FPVS-Studio-Setup-99.0.0.exe",
        download_url=(
            "https://github.com/zcm58/FPVS-Studio-2.0/releases/download/v99.0.0/"
            "FPVS-Studio-Setup-99.0.0.exe"
        ),
        version="99.0.0",
    ...
    def inspect_standalone() -> None:
        ...
        try:
            ...
            apply_window.setWindowTitle("FPVS Studio Updater — synthetic failure check")
