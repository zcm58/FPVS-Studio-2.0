...
from fpvs_studio.updates import (
    helper_client,
    helper_protocol,
    helper_runtime,
    helper_service,
...
from fpvs_studio.updates.helper_runtime import InstalledApplication, PreparedInstall
...
def _asset() -> InstallerAsset:
    ...
    return InstallerAsset(
        name,
        f"https://github.com/zcm58/FPVS-Studio-2.0/releases/download/v2.0.0/{name}",
        9,
...
def test_staging_is_independent_bounded_and_preserves_unknown_files(
    ...
    helper = tmp_path / "FPVS Studio Updater.exe"
...
def test_run_waits_for_parent_and_other_instances_before_launch(
    ...
    monkeypatch.setattr(
        helper_runtime, "studio_is_running", lambda exe: events.append("other") or True
    )
    prepared = PreparedInstall(_downloaded(tmp_path), installation, parent)
    with pytest.raises(UpdateError, match="Another Studio window"):
        prepared.run()
...
def test_run_ignores_cancellation_after_setup_and_restarts_only_verified_target(
    ...
    monkeypatch.setattr(helper_runtime, "studio_is_running", lambda exe: False)
    calls = []
    cancel = Event()
...
def test_install_session_lock_stays_held_while_setup_runs(
    ...
    monkeypatch.setattr(helper_runtime, "registered_installation", lambda: installation)
    monkeypatch.setattr(helper_runtime, "studio_is_running", lambda _: False)
...
def test_client_round_trip_through_real_private_subprocess(windowed_stdio: bool) -> None:
    ...
    script = """
import sys
from fpvs_studio.updates import helper_service
from fpvs_studio.updates.models import UpdateCheckResult, UpdatePhase
...
def test_client_cancellation_reaches_real_subprocess() -> None:
    script = """
from fpvs_studio.updates import helper_service
from fpvs_studio.updates.models import UpdatePhase, UpdateCancelled
def check(current_version, **kwargs):
    kwargs['phase_callback'](UpdatePhase('Ready for cancellation'))
    if kwargs['cancel_event'].wait(5):
        raise UpdateCancelled('Canceled by Studio')
...
"""
    client = helper_client.HelperClient(lambda *args: [sys.executable, "-c", script])
    cancel = Event()
    with pytest.raises(UpdateCancelled, match="Canceled by Studio"):
...
import sys
from pathlib import Path
from fpvs_studio.updates.helper_service import binary_stdio
from fpvs_studio.updates.helper_protocol import read_message, write_message
...
from threading import Event
from fpvs_studio.updates import helper_service
from fpvs_studio.updates.models import UpdatePhase
