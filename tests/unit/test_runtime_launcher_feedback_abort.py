...
def test_session_launch_aborts_cleanly_when_inter_block_break_is_cancelled(
    ...
    summary_columns = {cell.value: cell.column for cell in worksheet[1]}
    assert worksheet.cell(row=2, column=summary_columns["Aborted Y/N"]).value == "Y"
    assert worksheet.cell(row=2, column=summary_columns["Include In Analysis"]).value == "N"
...
def test_session_launch_exports_timing_violation_status_from_run_result(
    ...
    try:
        ...
        summary = launch_session(
            ...
            launch_settings=LaunchSettings(
                experiment_test_mode=True, serial_enabled=False,
                engine_name="stub-timing-violation",
            ),
        )
    finally:
